      ...
      <div class="card-body px-0 pt-0 pb-2">
        <div class="table-responsive p-0">
          <table v-if="!loading" class="table mb-0">
            ...
            <tbody>
              <tr v-for="item in ContractorsData" :key="item.id" class="">
                <td>
    ...
    <custom-modal ref="customModal" :title="modalTitle">
      ...
      <template v-slot:actions>
        <soft-button-vue :loading="loading" form="manger-form" type="submit">
          {{ editModeId ? "Save Client" : "Add Client" }}
        </soft-button-vue>
      </template>
    </custom-modal>
  ...
  data() {
    return {
      ...
      alertData: {
        ...
        alertTitle: "Alert",
        alertDescription:
          "After deleting this Client, you will not be able to recover it.",
      },
      statusAlertData: {
        icon: "fa fa-warning",
  ...
  methods: {
    ...
    async deletUser() {
      try {
        ...
        this.getContractorshandler();
        this.$notify({
          type: "warning",
          title: "Client",
          text: "Client deleted succesfuly",
        });
      } catch (err) {
        console.log(err);
        this.$notify({
    ...
    async getContractorshandler() {
      try {
        this.loading = true;
        const response = await api.get("/api/users/by-role/client/", {});
        this.ContractorsData = response.data;
        console.log("data", this.ContractorsData);
      } catch (err) {
        console.log(err);
      } finally {
        this.loading = false;
      ...
      try {
        ...
        this.$notify({
          ...
          text: !this.selectedStatus
            ? "Client set active successfully"
            : "Client set Inactive successfully",
        });
    ...
    async addNewManger() {
      try {
        ...
        this.$notify({
          type: "success",
          title: "Client",
          text: "Client added or updated succesfuly",
        });
